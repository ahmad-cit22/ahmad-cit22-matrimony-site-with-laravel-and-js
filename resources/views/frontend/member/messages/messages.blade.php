    ...
    <div class="attached-top bg-white border-bottom chat-header d-flex justify-content-between align-items-center p-3 shadow-sm">
        <div class="media align-items-center">
            ...
            <div class="media-body">
                <h6 class="fs-15 mb-1">
                    {{ $chat_thread->$user_to_show->user_id }}
    ...
    <div class="chat-info-wrap">
        ...
        <div class="chat-info c-scrollbar-light p-4 z-1">
            <div class="px-4 text-center mb-3">
                ...
                <h4 class="h5 mb-2 fw-600">{{ $chat_thread->$user_to_show->user_id }}</h4>
            </div>
            <div class="text-center">
                    ...
                    <h6 class="fs-13">
                        ...
                        @php
                            $present_address = \App\Models\Address::where('type', 'present')
                                ->where('user_id', $chat_thread->$user_to_show->id)
                                ->first();
                        @endphp
                ...
                <div class="text-center mb-3 px-3 mt-3">
                    <a @if (get_setting('full_profile_show_according_to_membership') == 1 && Auth::user()->membership == 1) href="javascript:void(0);" onclick="package_update_alert()"
                            @else
                                href="{{ route('member_profile', $chat_thread->$user_to_show->id) }}" @endif class="btn btn-block btn-soft-primary">{{ translate('View Full Profile') }}
                    </a>
                </div>
            </div>
        </div>
