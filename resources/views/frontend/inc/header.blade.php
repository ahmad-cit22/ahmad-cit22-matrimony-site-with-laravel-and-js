<div class="@if (get_setting('header_stikcy') == 'on') position-fixed @else position-absolute @endif w-100 top-0 z-1020">
                    ...
                    <ul class="list-inline mb-0 d-flex align-items-center justify-content-end ">
                        @auth
                            <li class="list-inline-item mr-3 pr-3 border-right text-reset opacity-60">
                                <span class="mr-3">{{ translate('Help Line:') }}</span>
                                <span>{{ get_setting('header_helpline_no') }}</span>
                            </li>
                            <li class="list-inline-item mr-3 pr-3 border-right text-reset opacity-60">
                                <span>{{ get_setting('email_address') }}</span>
                        ...
                        @if (Auth::check())
                            <li class="list-inline-item dropdown">
                                @php
                                    $notifications = \App\Models\Notification::latest()
                                        ...
                                        ->take(10)
                                        ->get();
                                    $unseen_notification = \App\Models\Notification::where('notifiable_id', Auth()->user()->id)
                                        ->where('read_at', null)
                                        ->count();
                                @endphp
                            ...
                            <li class="list-inline-item">
                                <a href="{{ route('logout') }}" class="btn btn-sm bg-primary-grad text-white fw-600 py-1 border">{{ translate('Logout') }}</a>
                            </li>
                        @else
                            ...
                            <li class="list-inline-item ml-3">
                                <a class="btn btn-sm bg-primary-grad text-white fw-600 py-1 border" href="{{ route('register') }}">{{ translate('Registration') }}</a>
                            </li>
                        @endif
                    </ul>
                </div>
    ...
    <header class="aiz-header shadow-md bg-white border-gray-300">
        <div class="aiz-navbar position-relative">
            <div class="container">
                <div class="d-xl-flex justify-content-between text-center text-xl-left">
                    <div class="logo">
                        <a href="{{ route('home') }}" class="d-inline-block py-10px">
                            @if (get_setting('header_logo') != null)
                                <img src="{{ uploaded_asset(get_setting('header_logo')) }}" alt="{{ env('APP_NAME') }}" class="" style="max-width: 400px;">
                            @else
                                <img src="{{ static_asset('assets/img/logo.png') }}" alt="{{ env('APP_NAME') }}" class="" style="max-width: 400px;">
                            @endif
                        </a>
                    </div>
                    <ul class="mb-0 pl-0 ml-lg-auto d-lg-flex align-items-stretch justify-content-center justify-content-xl-start mobile-hor-swipe">
                        <li class="d-inline-block d-lg-flex pb-1 {{ areActiveRoutes(['home'], 'bg-primary-grad') }}">
                            <a class="nav-link text-uppercase fw-700 fs-lg-15 d-flex align-items-center bg-white py-2" href="{{ route('home') }}">
                        ...
                        <li class="d-inline-block d-lg-flex pb-1 {{ areActiveRoutes(['member.listing'], 'bg-primary-grad') }}">
                            <a class="nav-link text-uppercase fw-700 fs-lg-15 d-flex align-items-center bg-white py-2" href="{{ route('member.listing') }}">
                        ...
                        <li class="d-inline-block d-lg-flex pb-1 {{ areActiveRoutes(['packages'], 'bg-primary-grad') }}">
                            <a class="nav-link text-uppercase fw-700 fs-lg-15 d-flex align-items-center bg-white py-2" href="{{ route('packages') }}">
                                <span class="text-primary-grad mb-n1">{{ translate('Premium Plans') }}</span>
                            </a>
                        </li>
                        <li class="d-inline-block d-lg-flex pb-1 {{ areActiveRoutes(['how.we.work'], 'bg-primary-grad') }}">
                            <a class="nav-link text-uppercase fw-700 fs-lg-15 d-flex align-items-center bg-white py-2" href="{{ route('how.we.work') }}">
                                <span class="text-primary-grad mb-n1">{{ translate('How We Work') }}</span>
                            </a>
                        </li>
                        <li class="d-inline-block d-lg-flex pb-1">
                            <a class="nav-link text-uppercase fw-700 fs-lg-15 d-flex align-items-center bg-white py-2" href="{{ url('/faq') }}">
                                <span class="text-primary-grad mb-n1">{{ translate('FAQ') }}</span>
                            </a>
                        </li>
                        <li class="d-inline-block d-lg-flex pb-1 {{ areActiveRoutes(['our_team'], 'bg-primary-grad') }}">
                            <a class="nav-link text-uppercase fw-700 fs-lg-15 d-flex align-items-center bg-white py-2" href="{{ route('our_team') }}">
                                <span class="text-primary-grad mb-n1">{{ translate('Our Team') }}</span>
                            </a>
                        </li>
                        <li class="d-inline-block d-lg-flex pb-1 {{ areActiveRoutes(['contact_us'], 'bg-primary-grad') }}">
                            <a class="nav-link text-uppercase fw-700 fs-lg-15 d-flex align-items-center bg-white py-2" href="{{ route('contact_us') }}">
            ...
            <div class="border-top d-none d-lg-block">
                <div class="container">
                    <ul class="list-inline d-flex align-items-center mb-0">
                        <li class="list-inline-item">
                            <a href="{{ route('dashboard') }}" class="text-reset d-inline-block px-4 py-3 fw-600 {{ areActiveRoutes(['dashboard'], 'text-primary-grad opacity-100') }}">
                                <i class="las la-tachometer-alt mr-1"></i>
                                <span>{{ translate('Dashboard') }}</span>
                            </a>
                        </li>
                        <li class="list-inline-item">
                            <a href="{{ route('profile_settings') }}" class="text-reset d-inline-block px-4 py-3 fw-600 {{ areActiveRoutes(['profile_settings'], 'text-primary-grad opacity-100') }}">
                                <i class="las la-user mr-1"></i>
                                <span>{{ translate('My Profile') }}</span>
                            </a>
                        </li>
                        <li class="list-inline-item">
                            <a href="{{ route('my_interests.index') }}" class="text-reset d-inline-block px-4 py-3 fw-600 {{ areActiveRoutes(['express-interest.index'], 'text-primary-grad opacity-100') }}">
                                <i class="la la-heart-o mr-1"></i>
                                <span>{{ translate('My Interest') }}</span>
                            </a>
                        </li>
                        <li class="list-inline-item">
                            <a href="{{ route('my_shortlists') }}" class="text-reset d-inline-block px-4 py-3 fw-600 {{ areActiveRoutes(['my_shortlists'], 'text-primary-grad opacity-100') }}">
                                <i class="las la-list mr-1"></i>
                                <span>{{ translate('Shortlist') }}</span>
                            </a>
                        </li>
                        <li class="list-inline-item">
                            <a href="{{ route('all.messages') }}" class="text-reset d-inline-block px-4 py-3 fw-600 {{ areActiveRoutes(['all.messages'], 'text-primary-grad opacity-100') }}">
                                <i class="las la-envelope mr-1"></i>
                                <span>{{ translate('Messaging') }}</span>
                            </a>
                        </li>
                        <li class="list-inline-item">
                            <a href="{{ route('my_ignored_list') }}" class="text-reset d-inline-block px-4 py-3 fw-600 {{ areActiveRoutes(['my_ignored_list'], 'text-primary-grad opacity-100') }}">
