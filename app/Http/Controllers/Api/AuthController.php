...
class AuthController extends Controller {
    ...
    public function signup(AuthRequest $request) {
        ...
        if (addon_activation('referral_system')) {
            $user = $user_service->store($request->only(['user_id', 'email', 'password', 'referral_code', 'phone']));
        } else {
            $user = $user_service->store($request->only(['user_id', 'email', 'password', 'phone']));
        }
        ...
        try {
            ...
            $notify_by = $user->id;
            $info_id = $user->id;
            $message = translate('A new member has been registered to your system. Name: ') . $user->user_id;
    ...
    //         }
    //         return response()->json(['result' => false, 'message' => translate('Wrong credentials given. Try again.'), 'user' => null], 401);
    //     }
    ...
    protected function loginSuccess($user) {
        ...
        if ($user->approved == 0) {
            auth()->logout();
            // return response()->json(['result' => false, 'message' => translate('Account not approved yet. Please contact us to register after choosing and paying for the membership type'), 'user' => null], 401);
            return back()->with('verifyError', 'Account not approved yet. Please contact us to register after choosing and paying for the membership type');
        }
