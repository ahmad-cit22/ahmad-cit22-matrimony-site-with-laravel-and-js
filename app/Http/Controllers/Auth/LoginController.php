...
class LoginController extends Controller {
    ...
    */
    ...
    public function login(Request $request) {
        ...
        if ($user) {
            if ($user->user_type == 'admin' || Carbon::parse($user->member->birthday)->age >= 18) {
                // if ($user->email_verified_at == null) {
                if (0) {
                    flash(translate('Please verify your account first.'))->error();
                ...
                $this->incrementLoginAttempts($request);
                flash(translate("Can't login until your age is 18 years or more."))->error();
                return redirect()->back();
            }
        } else {
            $this->incrementLoginAttempts($request);
